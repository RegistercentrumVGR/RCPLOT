...
#' @param satis_riket      Data frame with Swedish average satisfaction VAS
#'                           data.
...
#'                           corresponds to the first plot in alphabetical
#'                           order.
...
#' @param legend_position  Position of the legend in plot, matrix where
#'                           each row
  ...
  function(eq_vas_exp,
    ...
    get_ylim <- function(y_vec, y_break) {
      ...
      if (top - bottom < 3 * y_break) {
        range_size <- max(y_vec, na.rm = TRUE) + min(y_vec, na.rm = TRUE)
    ...
    if (is.vector(legend_position)) {
      legend_position <-
        matrix(rep(legend_position, length(subset)), nrow = length(subset))
    }
    ...
    for (i in subset) {
      ...
      eq_vas_plot <-
        ...
        theme_rc(
          title_hjust = 0,
          legend_position = legend_position[count, ],
          legend_justification = legend_position[count, ]
        ) +
        ggplot2::theme(
