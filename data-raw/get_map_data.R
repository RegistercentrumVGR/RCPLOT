...
municip_names <- codes[
  !is.na(codes$MunicipID),
  c("Name", "RegionID", "MunicipID")
]

municip_names[["id"]] <- paste0(
  municip_names[["RegionID"]],
  municip_names[["MunicipID"]]
)
...
save(counties,
  file = file.path("data", "counties.rda"),
  compress = "bzip2", ascii = TRUE
)
save(municipalities,
  file = file.path("data", "municipalities.rda"),
  compress = "bzip2", ascii = TRUE
)
